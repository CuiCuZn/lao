...
export const usePatientSubtitleTimeline = (options: SubtitleTimelineOptions) => {
  ...
  const logIncomingSubtitleMessage = (
    ...
    console.log('[patient-subtitle-message]', {
  ...
  const finalizeSentence = (message: ASRMessage) => {
    ...
    const item =
      ...
      (!message.translated && sentenceIndex >= 0
        ...
        : findActiveSourceItemForTranslation(message) || findRecentTranslatableRecord(message))
  ...
  const bindAsrStreams = (bindings: SubtitleTimelineStreamBinding[]) => {
    ...
    bindings.forEach((binding) => {
      ...
      const handler = (message: ASRMessage) => {
        // 这里的代码只注释 不删除
        // console.log(1111111111, message, binding.sourceLanguage, binding.targetLanguage);
